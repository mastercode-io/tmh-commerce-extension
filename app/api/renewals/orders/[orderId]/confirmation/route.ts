...
import { getRenewalConfirmation } from '@/lib/renewals/service';
...
export async function GET(
  ...
  try {
    const result = await getRenewalConfirmation({
      orderId,
      correlationId,
    });
    ...
    return createRenewalUnhandledErrorResponse(
      error,
      correlationId,
      'We hit a temporary problem while loading this renewal confirmation.',
    );
  }
}
