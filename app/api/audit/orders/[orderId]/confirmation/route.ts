...
import { getAuditConfirmation } from '@/lib/audit/service';
...
export async function GET(
  ...
  try {
    const result = await getAuditConfirmation({
      orderId,
      correlationId,
    });
    ...
    return createAuditUnhandledErrorResponse(
      error,
      correlationId,
      'We hit a temporary problem while loading this audit confirmation.',
    );
  }
}
